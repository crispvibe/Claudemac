...
<script setup>
  ...
  import { buildRoutePayload, isExternalRouteName, navigateByMenuName } from '@/utils/menuNavigation'
  ...
  const selectMenuItem = (index) => {
    if (!index || index === route.name || isExternalRouteName(index)) {
      return
    }
    ...
    const firstMenu = leftMenu.find(
      (item) => !item.hidden && !isExternalRouteName(item.path) && !isExternalRouteName(item.name)
    )
    if (!firstMenu) {
      return
    }
    router.push(buildRoutePayload(routerStore.routeMap, firstMenu.name))
