...
import (
	"fmt"
	"heyu/server/global"
	astutil "heyu/server/utils/goparse"
	"heyu/server/utils/stacktrace"
	"go.uber.org/zap"
...
func NewZapCore(level zapcore.Level) *ZapCore {
	...
	entity.Core = zapcore.NewCore(global.AppConfig.Zap.Encoder(), syncer, levelEnabler)
	return entity
}
...
func (z *ZapCore) WriteSyncer(formats ...string) zapcore.WriteSyncer {
	cutter := NewCutter(
		...
		z.level.String(),
		global.AppConfig.Zap.RetentionDay,
...
func (z *ZapCore) Write(entry zapcore.Entry, fields []zapcore.Field) error {
	for i := 0; i < len(fields); i++ {
		if fields[i].Key == "business" || fields[i].Key == "folder" || fields[i].Key == "directory" {
			...
			z.Core = zapcore.NewCore(global.AppConfig.Zap.Encoder(), syncer, z.level)
		}
	}
	// 先写入原日志目标
