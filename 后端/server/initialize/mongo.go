...
import (
	...
	"fmt"
	"heyu/server/global"
	"heyu/server/initialize/internal"
	"heyu/server/utils"
...
func (m *mongo) Initialization() error {
	...
	if global.AppConfig.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	mongoCfg := &qmgo.Config{
		Uri:              global.AppConfig.Mongo.Uri(),
		Coll:             global.AppConfig.Mongo.Coll,
		Database:         global.AppConfig.Mongo.Database,
		MinPoolSize:      &global.AppConfig.Mongo.MinPoolSize,
		MaxPoolSize:      &global.AppConfig.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &global.AppConfig.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &global.AppConfig.Mongo.ConnectTimeoutMs,
	}
	if global.AppConfig.Mongo.Username != "" && global.AppConfig.Mongo.Password != "" {
		mongoCfg.Auth = &qmgo.Credential{
			Username:   global.AppConfig.Mongo.Username,
			Password:   global.AppConfig.Mongo.Password,
			AuthSource: global.AppConfig.Mongo.AuthSource,
		}
	}
	client, err := qmgo.Open(ctx, mongoCfg, opts...)

	if err != nil {
		return errors.Wrap(err, "链接mongodb数据库失败!")
	}
	global.AppMongo = client
	err = m.Indexes(ctx)
	if err != nil {
		return err
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := global.AppMongo.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = global.AppMongo.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
