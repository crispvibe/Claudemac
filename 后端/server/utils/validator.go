...
func Verify(st interface{}, roleMap Rules) (err error) {
	...
	// 遍历结构体的所有字段
	for i := 0; i < num; i++ {
		fieldTag := typ.Field(i)
		val := val.Field(i)
		if fieldTag.Type.Kind() == reflect.Struct {
			if err = Verify(val.Interface(), roleMap); err != nil {
				return err
			}
		}
		if len(roleMap[fieldTag.Name]) > 0 {
			for _, v := range roleMap[fieldTag.Name] {
				switch {
				case v == "notEmpty":
					if isBlank(val) {
						return errors.New(fieldTag.Name + "值不能为空")
					}
				case strings.Split(v, "=")[0] == "regexp":
					if !regexpMatch(strings.Split(v, "=")[1], val.String()) {
						return errors.New(fieldTag.Name + "格式校验不通过")
					}
				case compareMap[strings.Split(v, "=")[0]]:
					if !compareVerify(val, v) {
						return errors.New(fieldTag.Name + "长度或值不在合法范围," + v)
					}
				}
			}
		}
