...
func (c *CloudflareR2) UploadFile(file *multipart.FileHeader) (fileUrl string, fileName string, err error) {
	...
	fileName = fmt.Sprintf("%s/%s", global.AppConfig.CloudflareR2.Path, fileKey)
	...
	input := &s3manager.UploadInput{
		Bucket: aws.String(global.AppConfig.CloudflareR2.Bucket),
	...
	return fmt.Sprintf("%s/%s", global.AppConfig.CloudflareR2.BaseURL,
			fileName),
		fileKey,
		nil
}
...
func (c *CloudflareR2) DeleteFile(key string) error {
	session := newSession()
	svc := s3.New(session)
	filename := global.AppConfig.CloudflareR2.Path + "/" + key
	bucket := global.AppConfig.CloudflareR2.Bucket
...
func (*CloudflareR2) newSession() *session.Session {
	endpoint := fmt.Sprintf("%s.r2.cloudflarestorage.com", global.AppConfig.CloudflareR2.AccountID)

	return session.Must(session.NewSession(&aws.Config{
		Region:   aws.String("auto"),
		Endpoint: aws.String(endpoint),
		Credentials: credentials.NewStaticCredentials(
			global.AppConfig.CloudflareR2.AccessKeyID,
			global.AppConfig.CloudflareR2.SecretAccessKey,
			"",
		),
	}))
}
